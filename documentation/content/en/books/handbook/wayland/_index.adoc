...
tags: ["Wayland", "XWayland", "KDE", "Plasma", "Xfce", "Gnome", "Intel", "AMD", "NVIDIA", "Wayfire", "Hikari", "Sway", "Hyprland", "Niri", "Labwc", "Riverwm", "dwl"]
showBookMenu: true
weight: 8
params:
...
Since Wayland is not an X server, traditional X screen connections will need to use other methods such as VNC or RDP for remote desktop management.
...
Volunteers to help work on the NVIDIA(R) DRM are requested. Kernel Modesetting (crossref:glossary[kms-glossary,KMS]) is required to run a Wayland environment on FreeBSD with NVIDIA(R) DRM.
====
...
Hyprland prioritizes delivering the latest Wayland-specfic features, with a focus on dynamic workspace management, eyecandy, and IPC, with a large plugin ecosystem. 

The compositor starts with a set amount of workspaces, each of which house windows that are auto-tiled according to user-defined workspace rules. 
Hyprland's workspaces are lined up horizontally, and can be grouped together depending on the display they should be on. 
Workspaces and monitors both support special workspaces, where a Wayland layer above the selected workspace can house additional windows which can be hidden or shown as needed, with those windows being grouped together.

Hyprland supports screen tearing and touchpad gestures for laptop users, as well as automatic configuration reloading upon changes.
The wiki page breaks down the configuration file into the functions of each section, but for this document, we will just consider 
a basic Hyprland installation.

A basic Hyprland installation is as follows, 
[source,shell]
----
# pkg install hyprland hyprpaper hyprpicker xdg-desktop-portal-hyprland kitty
----

[WARNING]
====
Hyprland's development team does not officially support NVIDIA hardware. However, the Hyprland wiki has instructions on how to configure Hyprland with NVIDIA hardware.
====
...
% start-hyprland
----
Ths will start Hyprland, which will auto-generate a configuration file at `~/.config/hypr/hyprland.conf`, and generates  a GUI prompt with instructions on how to configure the compositor for general use. A list of applications is listed that are both required for a functioning desktop, such as status bars, Polkit agents, and application launchers, and nice-to-haves like notification daemons, wallpaper utilities, and clipboard managers. 

The configuration is well-commented and has documentation on what each section does. For most users, the default workspaces work fine; however some important changes should be made. For example, to autostart the Firefox web browser when starting Hyprland, add 
[source,shell]
----
exec-once = firefox
----
You can use a `$` to create variables for use in multiple places, for example,
...
To launch an application via keybinds, for example
[.programlisting]
....
$mainMod = SUPER
bind = $mainMod, Q, exec, $terminal
....
will open the kitty terminal when pressing the kbd:[Q] key and the kbd:[L/super/Windows] key.

Since Hyprland automatically reads the newly edited configuration, the compositor does not need to restart to apply new changes. As Hyprland does not come with a status bar, it is recommended the user set up, at minimal, a status bar before migration. The wiki is a valuable resource that should be read before a migration, to understand how to create a full configuration file first. 

Locking the compositor in Hyprland requires the `hyprlock` package. The configuration is not created automatically, and is required or else `hyprlock` will not render anything, so create a configuraton file `hyprlock.conf` in `XDG_CONFIG_HOME` (`~/.config/hypr`). 
...
The `hyprlock` configuration will go into effect as soon as you update it, so execute
[source,shell]
----
% hyprlock
----
in a terminal to test your configuration

To exit Hyprland if required, execute
[source,shell]
----
% hyprctl dispatch exit
----
Many settings can be checked and changed using `hyprctl`, Hyprland's command-line interface. To display monitors for example,
[source,shell]
----
% hyprctl monitors
----
...
Labwc is a compositor that aims to bring the workflow and modularity of the Openbox X window manager to Wayland. It is very lightweight, and fully independent, with a focus on minimalism and simple stacking window management. It relies on clients for screenshots, wallpapers, etc to form a full desktop environment. It is also toolkit independent, so it does not require vast dependencies to be installed at runtime.

Since Labwc aims to be minimal, only a base install and configuration of Labwc is covered. Begin by installing the necessary packages,
[source,shell]
----
# pkg install labwc alacritty  
----

You will require `alacritty`, since at first launch Labwc will show an option to open a terminal using `alacritty` when right-clicking for further testing. 

Labwc configuration uses XML configuraton files. For example, to bind a keyboard shortcut to an action such as opening a terminal, create the file `~/.config/labwc/rc.xml` and add the following XML,
...
....
This will bind the Windows logo key and `q` to start `alacritty` and Windows logo key and `f` to start `firefox`.

The defaults for most users should be fine. The compositor will require a restart upon any configuration changes, which can by done by right-clicking anywhere and clicking exit.

Start the compositor by simply typing,

[source,shell]
----
% labwc
----
This starts the compositor. You can run Labwc in either a TTY or as a nested compositor inside another session, which is valuable for testing. Right click anywhere to open `alacritty`.

[[wayland-other]]
== Other compositors 
There are many other compositor available in the FreeBSD Ports Collection. A non-exhaustive list of compositors is provided below,

* dwl - dwm-like window manager for Wayland based on `wlroots`.
* Weston - reference design for Wayland compositors.
* Hikari - stacking window manger based on `wlroots` with special tiling and workspaces.
* Gamescope - microcompositor intended for isolating single applications within a nested session.
...
[NOTE]
====
The development version of Xwayland is rec ommended and was most likely installed with the Wayland package.
...
For the Hyprland compositor, Hyprland requires configuation for Xwayland in its config file, as follows,
[.programlisting]
....
...
env = XCURSOR_SIZE,32
....
This will ensure applications run properly in Xwayland while still preserving a uniform environment across your desktop.

For Labwc, support is built-in to the compositor, just the Xwayland binary is required. 
