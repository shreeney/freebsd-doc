...
% start-hyprland
----
Ths will start Hyprland, which will auto-generate a configuration file at `~/.config/hypr/hyprland.conf`, and generates  a GUI prompt with instructions on how to configure the compositor for general use. A list of applications is listed that are both required for a functioning desktop, such as status bars, Polkit agents, and application launchers, and nice-to-haves like notificaton daemons, wallpaper utilities, and clipboard managers. 
...
% hyprctl clients
----
Finally, many legacy X11-based programs do not have an interface for Wayland, so you will need to configure Xwayland. Hyprland requires configuation for Xwayland in its config file, as follows,
[.programlisting]
....
